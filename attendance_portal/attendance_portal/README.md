...
See `FILE_GUIDE.md` for a detailed, function-by-function explanation of every file —
written so both teammates (and anyone grading this) can follow exactly how data
...
**Batch verification testing** (for your report's numbers):
```
...
- **Member 1** — `model/pipeline.py` (data/training/evaluation logic) and
  `pages/1_Admin_Enrollment.py` (enrollment + model performance UI).
- **Member 2** — `pages/2_Login_Attendance.py` (login/verification UI) and
  `scripts/batch_verification_test.py` (verification testing).
- Shared interface: Member 2's page only ever calls `pipeline.predict()`,
  `pipeline.get_enrolled_students()`, and `pipeline.get_simulated_capture()` —
  it never needs to know how the model internals work.

## Known limitations (be upfront about these in the report/viva)

- **No live webcam yet.** "Captures" are held-out ORL images the model never
  trained on, standing in for a fresh photo. The architecture supports adding
  a real camera input later without changing the ML pipeline.
- **ORL identities stand in for real students.** This is a proof-of-concept
  built on a public benchmark dataset due to not having access to real student
  biometric data in the project timeframe — stated explicitly rather than
  implied.
- **Small sample size per identity** (10 images per ORL person) limits how
  large a held-out test set can be, which is why the batch verification script
  reuses held-out images across many trials.
